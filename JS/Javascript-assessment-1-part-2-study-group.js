...
function sumOfCubes(x, y) {
    ...
    if (isNaN(x) || isNaN(y)) {
        ...
        return (toTheThird(x) + toTheThird(y));
    }
}
...
function divide(x, y) {
    ...
    if (isNaN(x) || isNaN(y)) {
        ...
        return false;
    //    For specifying to get infinity or not
    } else {
        return x / y;
    }
}
...
// split splits a word into each individual character, while join joins them back together with no space between characters
